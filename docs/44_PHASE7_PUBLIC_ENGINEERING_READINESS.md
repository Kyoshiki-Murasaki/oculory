...
[Fresh repair CI cycle 2](https://github.com/Kyoshiki-Murasaki/oculory/actions/runs/29321469566) again passed both Ubuntu cells, macOS, deterministic offline experiments, packed-consumer install, and the Windows 10-pair stress step. Windows ordinary tests again passed 438 of 439, this time because the pre-existing cooperative stdin-close test read the immutable close record before Node delivered its optional asynchronous `process_exit` callback. The MCP client and test bytes were unchanged from cycle 1, where this exact test passed. The strengthened assertion retains immediate hard proof that the child and managed group are dead, then waits at most the existing one-second transcript deadline for the exit event and requires exit code `0` and null signal in both transcript and diagnostics. No production MCP behavior changed and the Windows test was not weakened or skipped. The third and final authorized repair cycle is authoritative for the exact final head.
