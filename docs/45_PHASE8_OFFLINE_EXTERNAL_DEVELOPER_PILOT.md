...
Doctor accepts Node 22.13.x or Node 24 and rejects other major lines. It checks npm, Git, compiled files, CLI availability, temp/space paths, output containment/writability, provider configuration absence, and the pinned CPython 3.12.13 Git MCP environment. The 33 behavior distributions match the reviewed Phase 7 versions exactly; only packaging bootstrap extras are allowed. Target version, source digest, and entry point are independently inspected.
