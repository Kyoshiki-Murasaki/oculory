...
import { ISSUE_SCENARIOS, issueScenariosByPartition } from './scenarios.js';
import { ISSUE_SYSTEM_PROMPT } from './policy.js';
import { recordIssueSession } from './record.js';
import { verifyAndNormalizeAllIssues, mineIssueIsolated } from './run.js';
import { evaluateIssueAssertion, evaluateIssueGoldenOutcome } from './verifier.js';

/**
 * Isolated issue-tracker model runs (Phase 5, docs/28). The task-server
 * model-run (src/pipeline/model-run.ts) is reused for all PURE reporting logic
 ...
 * issue-tracker versions here. Like the task/fs paths: a stub `ModelClient` in
 * tests, a real `OpenAiClient` on the CLI — NEVER an `OpenAiClient` constructed
 * here, so no test hits the network. Nothing auto-approves candidates.
 */
export interface IssueModelRunDeps {
  client: ModelClient;
}
...
async function recordIssueScenarios(
  store: RunStore,
  policy: ModelPolicy,
  ...
  outer: for (const scenario of opts.scenarios) {
    for (let trial = 0; trial < opts.trials; trial++) {
      try {
        const raw = await recordIssueSession({
          scenario,
          policy,
...
export function selectIssueScenarios(partition: PartitionSelector, maxScenarios: number | null): Scenario[] {
  const selected = partition === 'all' ? ISSUE_SCENARIOS : issueScenariosByPartition(partition);
  return maxScenarios !== null ? selected.slice(0, maxScenarios) : selected;
}

/* ============================== model-smoke ============================== */

export interface IssueModelSmokeOptions {
  runId: string;
  model: string;
  trials: number;
  budgetUsd: number;
...
export async function runIssueModelSmoke(store: RunStore, opts: IssueModelSmokeOptions, deps: IssueModelRunDeps): Promise<ModelSmokeSummary> {
  const scenarios = issueScenariosByPartition('smoke');
  const policy = new ModelPolicy({ client: deps.client, model: opts.model, budgetUsd: opts.budgetUsd, systemPrompt: ISSUE_SYSTEM_PROMPT });

  const recording = await recordIssueScenarios(store, policy, {
    scenarios,
    trials: opts.trials,
    budgetUsd: opts.budgetUsd,
  ...
  verifyAndNormalizeAllIssues(store);
  const instability = assessGroups(store, scenarios, policy.id);
  store.saveInstability(instability);

  const candidates = opts.mine ? mineIssueIsolated(store, instability) : [];
...
export interface IssueModelExperimentOptions {
  runId: string;
  model: string;
  trials: number;
  budgetUsd: number;
...
export async function runIssueModelExperiment(store: RunStore, opts: IssueModelExperimentOptions, deps: IssueModelRunDeps): Promise<ModelExperimentSummary> {
  const scenarios = selectIssueScenarios(opts.partition, opts.maxScenarios);
  const policy = new ModelPolicy({ client: deps.client, model: opts.model, budgetUsd: opts.budgetUsd, systemPrompt: ISSUE_SYSTEM_PROMPT });
  ...
  const candidates = opts.mine ? mineIssueIsolated(store, instability) : [];
  if (opts.mine) store.saveCandidates(candidates);
  if (opts.review) store.saveReport('review.md', renderReviewMarkdown(candidates, `Issue-tracker model experiment review — ${opts.runId}`));
...
export interface IssueModelReplayOptions {
  runId: string;
  model: string;
  trials: number;
  budgetUsd: number;
...
export async function runIssueModelReplay(store: RunStore, opts: IssueModelReplayOptions, deps: IssueModelRunDeps): Promise<ModelReplaySummary> {
  const policy = new ModelPolicy({ client: deps.client, model: opts.model, budgetUsd: opts.budgetUsd, systemPrompt: ISSUE_SYSTEM_PROMPT });
  ...
  outer: for (const test of opts.suite.tests) {
    const scenarios = ISSUE_SCENARIOS.filter((sc) => sc.family === test.scenario_family && partitions.includes(sc.partition));
    for (const scenario of scenarios) {
      const trials: ModelReplayTrial[] = [];
      for (let i = 0; i < opts.trials; i++) {
        try {
          const trace = await recordIssueSession({
            scenario,
            policy,
            mutationId: opts.mutationId ?? null,
          ...
          const ars = test.assertions.filter((a) => a.stable).map((a) => evaluateIssueAssertion(a, trace));
          ars.push(evaluateIssueGoldenOutcome(scenario, trace));
